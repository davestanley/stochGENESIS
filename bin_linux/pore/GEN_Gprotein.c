static char rcsid[] = "$Id: GENGprotein.c,v 1.2 2005/07/01 10:03:07 svitak Exp $";
...
/*GENGprotein.c contains the defining action function*/
...
#define Ss(field) GENGprotein->field

void GENGproteinPop(GENGprotein, action)

struct   GEN_GPROTEIN_type *GENGprotein;
Action   *action; 

{
	...
	if (debug > 1)
		ActionHeader("GENGproteinPop", GENGprotein, action);
	...
	case INIT:
        ...
        /**************************************************************************/
	case PROCESS:
		dt = Clockrate(GENGprotein);

	/*Update state*/
		MSGLOOP(GENGprotein, msg)
			{case 0:
 	...
 	/* Define new states */ 
		...
		/* Define array of transition probabilities */
					...
					// for G-protein, the concentration of G in the closed state is assumed to be
					// infinity. Therefore, it is assumed that only the concentration of R (using
					// the transmitter dummy variable in this case) matters.
			// Sn11 = 1;
			n[1] = (S11_S12 + T*TCoef1);
			n[2] = Sn11 * S11_S21;
		...
		if ( (tdest-Ss(telapsed)) < dt ){
			...
			/* Figure out which transition it will be */
				...
				randn = RAN();
				if (randn <= ns[1]) { Sn12++; }
				else if (randn <= ns[2]) { Sn21++; }

				else if (randn <= ns[3]) { Sn12--; }
	...
	to the first state on the top row of the chain that is farthest to the right and active*/
		if (Ss(openstate1) != 0){
			if (Ss(openstate1) == 11) Ss(No) = Ss(S11); else if (Ss(openstate1) == 21) Ss(No) = Ss(S21);
			else if (Ss(openstate1) == 12) Ss(No) = Ss(S12); else if (Ss(openstate1) == 22) Ss(No) = Ss(S22);
			else if (Ss(openstate1) == 13) Ss(No) = Ss(S13); else if (Ss(openstate1) == 23) Ss(No) = Ss(S23);
			else if (Ss(openstate1) == 14) Ss(No) = Ss(S14); else if (Ss(openstate1) == 24) Ss(No) = Ss(S24);
			else if (Ss(openstate1) == 15) Ss(No) = Ss(S15); else if (Ss(openstate1) == 25) Ss(No) = Ss(S25);
			else { printf ("Invalid entry for openstate1"); Ss(No) = Ss(S11); }
			if (Ss(openstate2) != 0){
				if (Ss(openstate2) == 11) Ss(No) = Ss(No) + Ss(S11); else if (Ss(openstate2) == 21) Ss(No) = Ss(No) + Ss(S21);
				else if (Ss(openstate2) == 12) Ss(No) = Ss(No) + Ss(S12); else if (Ss(openstate2) == 22) Ss(No) = Ss(No) + Ss(S22);
				else if (Ss(openstate2) == 13) Ss(No) = Ss(No) + Ss(S13); else if (Ss(openstate2) == 23) Ss(No) = Ss(No) + Ss(S23);
				else if (Ss(openstate2) == 14) Ss(No) = Ss(No) + Ss(S14); else if (Ss(openstate2) == 24) Ss(No) = Ss(No) + Ss(S24);
				else if (Ss(openstate2) == 15) Ss(No) = Ss(No) + Ss(S15); else if (Ss(openstate2) == 25) Ss(No) = Ss(No) + Ss(S25);
				else { printf ("Invalid entry for openstate1"); Ss(No) = Ss(S11); }
			}
		}

		else if ((S15_S14 + S15_S25) > 0) Ss(No) = Ss(S15);
		else if ((S14_S13 + S14_S24) > 0) Ss(No) = Ss(S14);
		else if ((S13_S12 + S13_S23) > 0) Ss(No) = Ss(S13);
		else if ((S12_S11 + S12_S22) > 0) Ss(No) = Ss(S12);
		else{
	...
	/*Update Current*/
		Ss(Gconcs) = Ss(No)*Ss(UnitG);		// Convert to units of concentration
		Ss(Gk) = Ss(Gbar)*( pow(Ss(Gconcs),Ss(nbinding_sites)) / ( pow(Ss(Gconcs),Ss(nbinding_sites)) + Ss(Kd) ) );
