...
// Plots the data to an graph, as well as saving it
// Uses "tight" fitting parameters


function PID_thresh2_plot (location, settle, duration, value, reset_trick, file_name, dest_plot)
...
//	setfield {local}/pid cmd {value} gain {1*{getfield {local} Cm}/{dt}}  tau_d {{dt}/40}  tau_i {{dt}*1e2} saturation 1e-4
	setfield {local}/pid cmd {value} gain 0.5e-7  tau_d {{dt}/4}  tau_i {{dt}} saturation 1e-4
