...
/*GENVhalf.c contains the defining action function*/
...
#define Ss(field) GENVhalf->field

void GENVhalfPop(GENVhalf, action)

struct   GENVhalf_type *GENVhalf;
Action   *action; 

{
	...
	double Vhalf, Vslope, tau, boltz, boltz_root, boltz_exp;
...
//    Vhalf = -0.030;		// V (some default values)
//    Vslope = 0.006;		// V
//    tau = 0.0045;		// s
    Vhalf = Ss(Vhalf);
    Vslope = Ss(Vslope);
    ...
    boltz_exp = 1.0/Ss(num_Xgate);
//    printf("boltz_exp = %f \n",boltz_exp);
    boltz_root = pow(boltz, boltz_exp);
    
    malpha = boltz_root/tau;
    mbeta = (1-boltz_root)/tau;
	...
	if (debug > 1)
		ActionHeader("GENVhalfPop", GENVhalf, action);
