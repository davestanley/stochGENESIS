...
/*GENsynpore.c contains the defining action function*/
...
#define Ss(field) GENsynpore->field

void GENsynPorePop(GENsynpore, action)

struct   GENsyn_pore_type *GENsynpore;
Action   *action; 

{
	...
	if (debug > 1)
		ActionHeader("GENsynporePop", GENsynpore, action);
	...
	case INIT:
        ...
        /**************************************************************************/
	case PROCESS:
		dt = Clockrate(GENsynpore);

	/*Update state*/
		MSGLOOP(GENsynpore, msg)
			{case 0:
