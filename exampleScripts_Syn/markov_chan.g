...
float onepS = 1e-12
float onepS_syn = 1e-12
float onepS_gap = 1e-12
...
function make_Na_diba_m					// Na channel from Diba-Koch 2004
        if (({exists Na_diba_m}))
                return
	end

        create GENvgpores Na_diba_m

                setfield ^ Ek {ENA} Ik 0 Gk 0 Nt 10 Gmax {20.0*{onepS}}

        setfield Na_diba_m malpha_A -6.37e3 malpha_B -0.182e6 malpha_C -1 malpha_D 0.035 malpha_F -9e-3	\
		mbeta_A 4.34e3 mbeta_B 0.124e6 mbeta_C -1 mbeta_D 0.035 mbeta_F 9e-3

        setfield Na_diba_m halpha_A -1.25e3 halpha_B -0.025e6 halpha_C -1 halpha_D 0.050 halpha_F -5e-3	\
		hbeta_A 0.6825e3 hbeta_B 0.0091e6 hbeta_C -1 hbeta_D 0.075 hbeta_F 5e-3	\
	    	num_Xgate 3 num_Ygate 1

	//Name of the equivalent non-markov channel that this is meant
	//to replace
	addfield Na_diba_m originalchannel
	setfield Na_diba_m originalchannel Na

end

function make_NaS_skinner_m					// Na channel from Saraga-Skinner 
        if (({exists NaS_skinner_m}))
                return
	end

        create GENvgpores NaS_skinner_m

                setfield ^ Ek {ENA} Ik 0 Gk 0 Nt 10 Gmax {20.0*{onepS}}


        setfield NaS_skinner_m malpha_A -3.8e3 malpha_B -0.1e6  malpha_C -1 malpha_D 0.038 malpha_F -10e-3 	\
		mbeta_A 4e3 mbeta_B 0 mbeta_C 0 mbeta_D 0.065 mbeta_F 18e-3

        setfield NaS_skinner_m halpha_A 0.07e3 halpha_B 0 halpha_C 0 halpha_D 0.063 halpha_F -20e-3	\
		hbeta_A 1e3 hbeta_B 0 hbeta_C 1 hbeta_D 0.033 hbeta_F -10e-3	\
	    	num_Xgate 3 num_Ygate 1

	//Name of the equivalent non-markov channel that this is meant
	//to replace
	addfield NaS_skinner_m originalchannel
	setfield NaS_skinner_m originalchannel NaS_skinner

end


function make_Na_skinner_m					// Na channel from Saraga-Skinner 
        if (({exists Na_skinner_m}))
                return
	end

        create GENvgpores Na_skinner_m

                setfield ^ Ek {ENA} Ik 0 Gk 0 Nt 10 Gmax {20.0*{onepS}}


        setfield Na_skinner_m malpha_A -4.5e3 malpha_B -0.1e6  malpha_C -1 malpha_D 0.045 malpha_F -10e-3 	\
		mbeta_A 4e3 mbeta_B 0 mbeta_C 0 mbeta_D 0.070 mbeta_F 18e-3

        setfield Na_skinner_m halpha_A 0.07e3 halpha_B 0 halpha_C 0 halpha_D 0.070 halpha_F -20e-3	\
		hbeta_A 1e3 hbeta_B 0 hbeta_C 1 hbeta_D 0.040 hbeta_F -10e-3	\
	    	num_Xgate 3 num_Ygate 1


	//Name of the equivalent non-markov channel that this is meant
	//to replace
	addfield Na_skinner_m originalchannel
	setfield Na_skinner_m originalchannel Na_skinner
...
function make_Na_axon_m_active
	...
	addfield NaA_m originalchannel
	setfield NaA_m originalchannel NaA
	addfield NaA_m isactive
	setfield NaA_m isactive active
	...
	addfield ../NaA isactive
	setfield ../NaA isactive active
...
//========================================================================
//                T-type Ca channel (incomplete!)
//========================================================================

function make_Ca_T_m
        if (({exists Ca_T_m}))
                return
        end

        create GENvgpores Ca_T_m

                setfield ^ Ek {ECA} Ik 0 Gk 0 Nt 10 Gmax {8.0*{onepS}}

        setfield Ca_T_m malpha_A 1.6e3 malpha_B 0 malpha_C 1.0 malpha_D {-1.0*(0.065 + EREST_ACT)}  \
            malpha_F -0.01389 mbeta_A {-20e3*(0.0511 + EREST_ACT)} mbeta_B 20e3 mbeta_C -1.0  \
            mbeta_D {-1.0*(0.0511 + EREST_ACT)} mbeta_F 5.0e-3 \
	    num_Xgate 2 num_Ygate 0
	...
	//Name of the equivalent non-markov channel that this is meant
	//to replace
	addfield Ca_T_m originalchannel
	setfield Ca_T_m originalchannel Ca_T
...
function make_K_AHP_m (location)
	...
	if (location == "1"); str KAHPname = "K_AHP"
        ...
        create GENtabchan {KAHPname}_m
                ...
                //      S
		if ((location == "1") || (location == "2"))
	                setfield ^ Ek {pyr_EK} Nt 10 Gmax {10.0*{onepS}} Ik 0 Gk 0 num_Xgate 1
		elif ((location == "3") || (location == "4"))
	                setfield ^ Ek {int_EK} Nt 10 Gmax {10.0*{onepS}} Ik 0 Gk 0 num_Xgate 1
		end
...
function make_AMPA_m
	...
	setfield AMPA_m Ek {E_AMPA} Nt 10 Gmax {10*{onepS_syn}} Trans 5e-2	\
		S11_S12 0 TCoef2 {Rb}	\
...
// This channel is recommended over the channel proposed in Koch's text
// because Koch's version takes excessively long to recover from desensitization.
// Fast version for interneurons
//
//========================================================================

//function make_AMPAi_m_stevens		//Fast version for interneurons
//
//	float c0c1 = {4.59e6 * 2}
//	float c1c0 = {4.26e3 * 2}
//	float c1c2 = {2.84e7 * 2}
...
//		TCoef5 {c3c4} openstate1 14
//
//	addfield AMPAi_m originalchannel
//	setfield AMPAi_m originalchannel AMPAi
//end
...
float Rc = 73.8
	...
	setfield NMDA_m Ek {E_NMDA} Nt 10 Gmax {45*{onepS_syn}} Trans 9.3e-6 \
...
function make_GABA_A_m
	...
	setfield GABA_A_m Ek {E_GABAA} Gmax {20*{onepS_syn}} Trans 9.2e-5 \
...
////Optional - other constats found in Table 3
//float r1 = 18
//float r2 = 4.4
//float r3 = 1.4
//float r4 = 1.7
...
int k_pore_mode = 1
	...
	// 3: Model as channel with deactivation (as per Destexhe 1994 eq 24 and Fig 8)
	...
	// Model K_pore as 4 independent binding events
		...
		setfield GABA_B_m/K_pore Ek {E_GABAB} Gmax {10*{onepS_syn}} malpha_A 0 malpha_B {rate_per_mols} malpha_C 0 malpha_D 0 malpha_F 1e9	\
	...
	// Model K_pore as 4 squential binding events (binding sites are specific with specific order)
		...
		setfield GABA_B_m/K_pore Ek {E_GABAB} Gmax {10*{onepS_syn}} TCoef1 {rate_per_mols} S12_S11 {{rate_per_mols}*{Kd_root4}}	\
	...
	//Model as channel with deactivation (as per Destexhe eq 24)
		create GENsynpores GABA_B_m/K_pore
		setfield GABA_B_m/K_pore Ek {E_GABAB} Gmax {10*{onepS_syn}} TCoef1 {{Gprotein_renormalization}*{r1}} S12_S11 {r2}	\
...
float Rc = 73.8
	...
	//Create Channel
	create GENsynpores GABA_B_m
	setfield GABA_B_m Ek {E_GABAB} Nt 10 Gmax {45*{onepS_syn}} Trans 9.3e-6 \
				TCoef1 {Rb} TCoef2 {Rb} S12_S11 {Ru} S13_S12 {Ru} \
				S13_S23 {Rr} S23_S13 {Rd} S13_S14 {Ro} S14_S13 {Rc} \
				openstate1 14
...
function make_GAP_m
	...
	create GENgap GAP_m
	setfield GAP_m Gmax {55*{onepS_gap}} \
...
function make_interneuron_m
	...
	make_K_Ai_m  
	make_Ca_m
	make_K_C_m 4; make_K_C_m 3
	make_K_AHP_m 4; make_K_AHP_m 3;
	make_leak 2
	make_leak_m 2
