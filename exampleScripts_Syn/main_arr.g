...
int i
foreach i ( 8 18 28 38 48 58 68 )
	echo {i}
	create asc_file pyr{i}
	setfield ^  flush 0 leave_open 1 append 0 notime 0
	useclock ^ 1
	addmsg {pyr_array_name}{pyr_cell_name}[{i}]/ax pyr{i} SAVE Vm
	addmsg {pyr_array_name}{pyr_cell_name}[{i}]/soma pyr{i} SAVE Vm
end

foreach i ( 3 5 7 )
	echo {i}
	create asc_file fb{i}
	setfield ^  flush 0 leave_open 1 append 0 notime 0
	useclock ^ 1
	addmsg {fb_array_name}{fb_cell_name}[{i}]/ax fb{i} SAVE Vm	
	addmsg {fb_array_name}{fb_cell_name}[{i}]/soma fb{i} SAVE Vm
