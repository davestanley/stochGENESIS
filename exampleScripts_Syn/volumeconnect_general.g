...
function volumeweight_markov (src, dest, fixed, uniform)	//Standard form for volumeweight, only use this for markov synapses
	str curr_src
	str curr_dest
	str dest_list
	int i
			...
			if ( {exists {curr_dest}/.. Nt} )	//Make sure it's a markov element
				foreach dest_list ({el {dest}})
					if ({el {curr_dest}/.. } == {el {dest_list} }   ) //Make sure it's in the destination list
						...
						//echo Current Nt={getfield {curr_dest}/.. Nt} setfield {curr_dest}/.. Nt {{getfield {curr_dest}/.. Nt} * {final_weight}}
						setfield {curr_dest}/.. Nt {trunc {{getfield {curr_dest}/.. Nt} * {final_weight}}}
