...
// Inhibitory Cell, Journal of Computational Neuroscience, 2, 291-298 (1995)
		...
		//Edit by Dave 2009
			...
			//Note that I use the suffix _m, but really this is not a markov version. See
			//the markov_chan.g file for the correct markovian implementation.
...
function make_K_C
	...
	if (mode == 0)
		...
		call K_C/tab TABFILL 90000 0
		addmsg K_C/tab K_C MULTGATE output 1
        	setfield  K_C sendmsg1 "../Ca_conc  tab INPUT Ca"
	else
		create tabgate K_C/tab
		call K_C/tab TABCREATE alpha {xdivs} {xmin} {xmax}
		call K_C/tab TABCREATE beta {xdivs} {xmin} {xmax}
		...
		for (i = 0; i <= (xdivs); i = i + 1)
		    ...
		    setfield K_C/tab alpha->table[{i}] {alpha}
		    setfield K_C/tab beta->table[{i}] {beta}
		    x = x + dx
		end

		setfield K_C/tab alpha->calc_mode 0 beta->calc_mode 0
		call K_C/tab TABFILL alpha 90000 0
		call K_C/tab TABFILL beta 90000 0
		addmsg K_C/tab K_C MULTGATE m 1
        	setfield  K_C sendmsg1 "../Ca_conc  tab VOLTAGE Ca"
	end
	...
	call K_C/X TABFILL beta 3000 0
        ...
        setfield  K_C sendmsg2 "..  X  VOLTAGE Vm"
end
...
function make_K_C_soma
	...
	if (mode == 0)
		...
		call K_Cs/tab TABFILL 90000 0
		addmsg K_Cs/tab K_Cs MULTGATE output 1
        	setfield  K_Cs sendmsg1 "../Ca_concs  tab INPUT Ca"
	else
		create tabgate K_Cs/tab
		call K_Cs/tab TABCREATE alpha {xdivs} {xmin} {xmax}
		call K_Cs/tab TABCREATE beta {xdivs} {xmin} {xmax}
		...
		for (i = 0; i <= (xdivs); i = i + 1)
		    ...
		    setfield K_Cs/tab alpha->table[{i}] {alpha}
		    setfield K_Cs/tab beta->table[{i}] {beta}
		    x = x + dx
		end

		setfield K_Cs/tab alpha->calc_mode 0 beta->calc_mode 0
		call K_Cs/tab TABFILL alpha 90000 0
		call K_Cs/tab TABFILL beta 90000 0
		addmsg K_Cs/tab K_Cs MULTGATE m 1
        	setfield  K_Cs sendmsg1 "../Ca_concs  tab VOLTAGE Ca"
	end
        ...
        setfield  K_Cs sendmsg2 "..  X  VOLTAGE Vm"
end
...
function make_K_C_somai
        ...
        call K_Csi/tab TABFILL 90000 0
        float xmin = -0.1
        float xmax = 0.05
        int xdivs = 49
